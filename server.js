...
async function handle(message) {
  ...
  if (method === 'initialize') {
    send({ jsonrpc: '2.0', id, result: { protocolVersion: params.protocolVersion || '2025-06-18', capabilities: { tools: {} }, serverInfo: { name: 'board-local', version: '1.1.0' } } });
    return;
  }
  ...
  try {
    ...
    const data = await callBoard(name, model);
